...
.config(['$locationProvider', '$routeProvider', function($locationProvider, $routeProvider) {
 ...
 $routeProvider.when('/', {
    ...
    controller: 'HomeCtrl',
    resolve: {
    	EventData: ['$q', '$http', 'GuestList',  function($q, $http, GuestList) {
          return $http
              .get('/', {} )
              .then(function(response){
                  var initData = response.data
                  GuestList.guests = initData.guests
                  return initData;
              })
        }]
    }
...
.factory('AuthInterceptor', [ function AuthInterceptor(){
    ...
    // adds auth token to every call
    // IRL would only do this if user was authenticated / token existed in localstorage
    function addToken(config) {
	    ...
	    var token = _base64.encode('admin@test.com' + ":" + 'secret')
